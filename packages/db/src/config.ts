...
export function dbUrlsFromEnv(env: NodeJS.ProcessEnv = process.env): DbUrls {
  return {
    admin: env.ADMIN_DATABASE_URL ?? 'postgres://postgres:postgres@127.0.0.1:5432/fluvia',
    app:
      env.APP_DATABASE_URL ?? 'postgres://fluvia_app:fluvia_app_dev_password@127.0.0.1:5432/fluvia',
    worker:

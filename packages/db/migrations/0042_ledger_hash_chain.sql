...
-- extensión futura. En una copia restaurada, un candidato pendiente referencia
-- txids del cluster ORIGINAL — inofensivo (la copia está quiescente; a lo sumo
-- sella un segmento válido).
--
...
BEGIN
  ...
  IF FOUND THEN
    IF cand.upto_seq <= last_upto THEN
      -- Candidato obsoleto (nunca debería pasar con un solo escritor).
      DELETE FROM ledger_checkpoint_candidates WHERE id <= cand.id;
    ELSIF pg_snapshot_xmin(pg_current_snapshot()) >= cand.horizon_txid
          AND now() - cand.created_at >= min_candidate_age THEN
  ...
  -- Fase 1: registrar el siguiente candidato (si hay asientos sin sellar y
  ...
  IF NOT EXISTS (SELECT 1 FROM ledger_checkpoint_candidates) THEN
    ...
    IF new_upto IS NOT NULL AND new_upto > last_upto THEN
      INSERT INTO ledger_checkpoint_candidates (upto_seq, horizon_txid)
      VALUES (new_upto, pg_snapshot_xmax(pg_current_snapshot()));
    END IF;
  END IF;

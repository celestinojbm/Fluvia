...
| `worker-down` | ✅ **PASS (5/5)** | `pnpm --filter @fluvia/api run drill:worker-down` (`apps/api/drills/worker-down-drill.ts`) — reinicio (`checkReady`) → el relay/inbox drenan sus backlogs solos (delivered/processed) → el drift watcher detecta el drift acumulado → `rebuildProjection` (reparación explícita) lo limpia |

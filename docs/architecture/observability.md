...
| `fluvia_ledger_chain_checkpoints`             | gauge   | — (F6, 0042; checkpoints sellados del hash-chain de tamper-evidence — una CAÍDA = cadena borrada) |
| `fluvia_ledger_chain_sealed_upto_seq`         | gauge   | — (último `seq` de `ledger_entries` cubierto; lo posterior es horizonte pendiente, ~2 intervalos) |
...
| Cadena del ledger encogida         | `delta(fluvia_ledger_chain_checkpoints[15m]) < 0` o `delta(fluvia_ledger_chain_sealed_upto_seq[15m]) < 0`                                                                   | CRÍTICA   | Los checkpoints son append-only: estos gauges SOLO pueden crecer. Una caída = checkpoints BORRADOS — incluido el único ataque que el check [7] de invariantes no ve en-BD (borrar la cadena ENTERA deja cero checkpoints = verde trivial). Incidente de integridad: el borrado exige tier superusuario (F6, threat model §5); el anclaje EXTERNO del chain_hash es la extensión |

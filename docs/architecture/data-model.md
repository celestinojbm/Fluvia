...
- **Conciliación/operaciones (F4-01a/F4-03a)**: `settlement_reports`, `settlement_lines`, `reconciliation_entries` (RLS + append-only) y `operational_cases` (RLS, mutable con ciclo `open→acknowledged→resolved`; sin DELETE). Un trigger materializa un caso por cada discrepancia (`reconciliation_entries` != matched). `risk_decisions` y el `reconciliation_batches`/`_items` conceptual del diseño llegan más adelante.

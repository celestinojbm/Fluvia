...
- **Conciliación/operaciones (F4-01a/F4-03a/F4-03b)**: `settlement_reports`, `settlement_lines`, `reconciliation_entries` (RLS + append-only), `operational_cases` (RLS, mutable con ciclo `open→acknowledged→resolved`; sin DELETE) y `case_adjustments` (RLS, mutable `proposed→applied|rejected`; four-eyes por CHECK `approved_by <> proposed_by`; sin DELETE). Un trigger materializa un caso por cada discrepancia (`reconciliation_entries` != matched); un caso aprobado postea un asiento compensatorio `recon.differences ↔ suspense`. `risk_decisions` y el `reconciliation_batches`/`_items` conceptual del diseño llegan más adelante.

...
Cotas de tiempo (RA-F6-001): la transacción del claim+efecto corre por `withTenantTransaction` (V2-R1), así que dentro de ella quedan activas las TRES cotas como SET LOCAL — `statement_timeout` (default 30 s), `lock_timeout` (default 3 s en esta capa) e `idle_in_transaction_session_timeout` (default 60 s). `statementTimeoutMs` debe ser > `lockTimeoutMs` para que un claim bloqueado agote primero el lock (55P03 → `409 processing_in_flight`) y nunca surja un 57014 crudo. Un handler patológico o una sesión idle dentro de la tx no puede acaparar una conexión del pool sin límite.

...
> **Estado de implementación (2026-07-05, F3-01/F3-02): PARCIAL.** Construido hoy: plano contable del ciclo (F2-03/F2-04), `payment_intents` REAL con FSM en el motor + `payment_attempts` (0017, F3-01) y los primeros endpoints idempotentes del plano de integración: `POST/GET /v1/payment_intents`, `GET /v1/payment_intents/:id`, `POST .../cancel` (F3-02, contrato en `docs/api/openapi.v1.json`). Construido también (F3-03 parte síncrona): **confirm** idempotente con contrato asíncrono (responde `processing`; el estado final se lee vía GET), MockPaymentProvider (tokens de prueba), attempts en ejecución con captura contable ATÓMICA (asiento+attempt+intent+`amount_captured` en una transacción vía `onPosted`; fees 0 hasta PEND-002) y timeout→`indeterminate` (V4 §23: sin resolución por asunción). Construido también (F3-03b): flujo asíncrono completo — `tok_pse` ⇒ attempt `submitted`; el proveedor notifica por webhook FIRMADO a `/v1/providers/mock/webhook` (HMAC + tolerancia + dedup race-safe); el `InboxProcessor` del worker (primer handler real de F2-12) resuelve `submitted`/`indeterminate` por fuente verificada con la MISMA captura contable atómica; los webhooks tardíos/contradictorios quedan `ignored_out_of_order` sin re-abrir nada. Construido también (F3-04): barrido `submitting→indeterminate` con lease de 5 min (`sweep_payment_attempts()`, auditoría atómica), alerta de indeterminados envejecidos (>30 min), timeout duro del adapter y circuit breaker por proveedor (`ResilientProvider`; circuito abierto ⇒ fallo limpio `provider_unavailable` — jamás se envió, jamás ambiguo). NO construido: checkout, webhooks salientes (F3-07), customers (F3-05), conciliación (F4).

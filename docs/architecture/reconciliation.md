...
**F4-03c** expone la operación por SESIÓN (plano humano del panel): rutas `/v1/organizations/:orgId/operational_cases/*` (list/detail + acknowledge/resolve) y `/case_adjustments/*` (propose/approve/reject), bajo el permiso RBAC nuevo `reconciliation:manage` (owner/admin/finance). El **four-eyes es REAL sobre HTTP**: dos miembros distintos con el permiso — U1 propone, U1 no puede aprobar (409 `four_eyes_required`), U2 aprueba y aplica. Una API key jamás alcanza este plano (autorizar dinero es humano). Falta solo la UI del panel (F4-03c-ii).

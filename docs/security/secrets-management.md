...
6. Local: `.env` (gitignored) + `.env.example` sin valores reales. CI: secretos del runner. Cloud: secret manager del proveedor **inyectado como variables de entorno al arranque** (la app conserva su contrato de env validado + anti-mezcla; sin SDK de proveedor en runtime) — enfoque decidido en [ADR-0012](../adr/0012-secret-manager-produccion.md) (Propuesto; el vendor concreto se fija en el despliegue, F5).

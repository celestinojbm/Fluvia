...
| AUD-P1-004 (outbox sin relay) | PLANIFICADO (siguiente incremento) | — | F2-11 es el próximo incremento del roadmap; absorbe AUD-P1-007 y AUD-P2-005 como criterios de aceptación |
| AUD-P1-005 (inbox/webhooks entrantes) | PLANIFICADO | — | F2-12, inmediatamente tras F2-11. Bloquea MockProvider asíncrono (F3-03) |
...
| AUD-P1-007 (worker BYPASSRLS/grants) | **MITIGADO** (lote AUD-1); rediseño en F2-11 | 2026-07-04 | Migración `0008` §3: `REVOKE ALL` al worker sobre `ledger_accounts`, `ledger_transactions`, `ledger_entries`, `balance_projections`, `api_keys`. Meta-test de privilegios existente en `packages/db/test/tenant-escape.test.ts` cubre la matriz. El rol mínimo definitivo del relay (sin BYPASSRLS amplio) llega con F2-11 + ADR-0011 |
...
| AUD-P2-005 (outbox sin envelope) | PLANIFICADO (F2-11) | — | Criterio de aceptación de F2-11: envelope común `event_id, schema_version, occurred_at, producer, resource` en TODO evento del outbox |
...
- Lote AUD-1: `pendiente de push — se registra aquí el run al confirmarse verde`.

## Criterio de re-auditoría (adoptado del auditor)

Solicitar re-auditoría al cerrar P1-003, P1-004, P1-005, P1-006 y el rediseño completo de P1-007 — estimado: final de Fase 2.

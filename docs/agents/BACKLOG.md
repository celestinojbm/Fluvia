...
| F1-04 | Auth: registro, verificación email, login, MFA TOTP, sesiones revocables, RBAC, API keys con scopes | Identidad | F1-03 | Alto | XL | Suite authN/authZ + BOLA por endpoint; step-up para acciones sensibles | **Parcial**: sub-entrega (a) completada 2026-07-04 — registro+verificación+login scrypt+lockout+sesiones revocables+rol fluvia_auth (38 tests). Pendiente (b) MFA/step-up, (c) API keys scopes+RBAC |

...
**F2-11 — outbox relay worker** (`FOR UPDATE SKIP LOCKED`, backoff+jitter, DLQ, replay auditado, sin doble entrega con 2 workers), **absorbiendo AUD-P1-007** (rol relay dedicado de privilegio mínimo, sin BYPASSRLS amplio — ADR-0011) **y AUD-P2-005** (envelope común `event_id, schema_version, occurred_at, producer, resource`): da vida a los eventos que el ledger ya emite y desbloquea el inbox (F2-12) y los webhooks (F3-07). Después: F2-12 → F2-09/10 (sobre la tabla ya corregida) → F2-05/06/08 (cierran Gate Ledger).

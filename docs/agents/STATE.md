...
| MFA/step-up (F1-04b), observabilidad (F1-07), taxonomía errores (F1-08), purga (F1-09), seeds (F1-10), inbox (F2-12), API de pagos, checkout, webhooks salientes… | No construido |
...
**F2-12 — inbox durable `provider_events`** (dedup por `(provider, provider_event_id)`, raw firmado verificado, respuesta solo tras COMMIT, procesamiento asíncrono con validación Zod y DLQ redactada): cierra AUD-P1-005 y prepara el MockProvider asíncrono (F3-03). Después: F2-09/10 (idempotencia API sobre la tabla ya corregida) → F2-05/06/08 (cierran Gate Ledger) → F2-07 (compensaciones vía servicio).

...
**F2-11 — outbox relay worker** (`FOR UPDATE SKIP LOCKED`, backoff+jitter, DLQ, replay auditado, sin doble entrega con 2 workers): da vida a los eventos que el ledger ya emite y desbloquea el inbox (F2-12) y los webhooks (F3-07). Alternativa: F2-08 (suite de concurrencia formal) o F2-05/F2-06 (rebuild+drift check e invariantes SQL externas) para cerrar el Gate Ledger.

...
| 29 | **Opción B — preparar Fase 5 sin iniciarla** (propietario, 2026-07-11). Baseline actual: `claude/new-session-haeo7h` @ `eb2d570f3ce4d2b982c500e11a4d361b58040c7f` (F6 aprobada para sandbox/hardening, decisión #28). Decisión: registrar un **plan documental** de preparación de Fase 5 en `docs/phases/f5-preparation-plan.md` (planificación estratégica, NO implementación). **Fase 5 NO iniciada.** Las etapas **F5.0 (verificación legal/matriz) → F5.1 (proveedor) → F5.2 (infra/secret manager) → F5.3 (adaptador real + field-level enc) → F5.4 (exposición: PEND-006 luego PEND-004)** requieren, cada una, una **autorización humana explícita y separada** del propietario. Primera inversión recomendada: solo F5.0 (sin código). NO autoriza producción, `live`, proveedor real, exposición pública ni sandbox compartido; freeze #24 sigue vigente | Gobernanza | `docs/phases/f5-preparation-plan.md` |
| 30 | **F5.0 — investigación legal/compliance autorizada (solo documental)** (propietario, 2026-07-11). Baseline: `claude/new-session-haeo7h` @ `9d433002f12aa098aca5a54ebd2f08c5a32717fe`. Decisión: ejecutar F5.0 como **paquete de investigación** con fuentes oficiales/primarias (URL + fecha de consulta 2026-07-11) y estado epistémico por afirmación, **sin conclusiones legales definitivas**. Entregables: `docs/compliance/f5-0-legal-research.md`, `docs/compliance/legal-questions-checklist.md`, y la columna «Verificación» de `jurisdiction-matrix.md` con fuente/fecha/estado — **ninguna fila cerrada legalmente**. Hallazgo bisagra (pregunta-para-abogado, NO conclusión): las «pasarelas de pago» no son vigiladas por la SFC per se; el régimen de adquirente no vigilado (Decreto 1692/2020) exige segregación de fondos si se **recauda** — la clasificación de Fluvia (no-custodio) es pregunta para abogado. **F5.1–F5.4 NO autorizadas.** No código, no proveedor real, no `live`, no producción, no exposición, no sandbox compartido; freeze #24 vigente. Salida esperada: llevar `legal-questions-checklist.md` a un abogado colombiano | Gobernanza | `docs/compliance/f5-0-legal-research.md` |
| 31 | **F6.5 «Product Sandbox Completion» autorizada como etapa de SANDBOX CERRADO** (propietario, 2026-07-11). Baseline: `claude/new-session-haeo7h` @ `0154c34d10346564b4076a829a8045fba88c86d1`. Alcance: completar la **superficie de producto** (dashboard de pagos/refunds/links/sessions, webhooks + API keys UI, onboarding mock, demo data showroom, export CSV de lectura, docs de uso) **consumiendo la API existente, sin tocar el núcleo auditado**. **El primer PR es docs-only** (este plan). **F6.5A–D se ejecutan en PRs separados y cada uno requiere autorización individual del propietario.** Límites: **F5.1–F5.4 siguen bloqueadas** · MockProvider único proveedor · NO proveedor real · NO credenciales reales · NO `live` · NO exposición pública · NO sandbox compartido · NO producción · freeze #24 vigente · **badge «SANDBOX — dinero simulado» obligatorio en toda UI nueva** · no tocar `docs/audits/independent-audit-v1/`. Plan y guardrails en `docs/phases/f6-5-sandbox-product-plan.md` | Gobernanza | `docs/phases/f6-5-sandbox-product-plan.md` |

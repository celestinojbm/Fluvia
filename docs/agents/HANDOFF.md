...
**Próximo incremento recomendado**: quedan las opciones GRANDES de F6, todas de carácter distinto y sin dependencia entre sí: **security review formal** (una pasada de auditoría de seguridad completa sobre el sistema endurecido — el candidato natural ahora que el backlog §5 buildeable en sandbox está casi agotado), o **load-soak-chaos** (pruebas de carga/resiliencia — necesita algo de infra/tooling; cuidar el Nivel A: no declarar resultados simulados). El resto del residual §5 está acoplado a infra/despliegue (F5: cifrado field-level de credenciales de proveedor; `trustProxy` acotado y demás → PEND-006) o es del operador (copia OFFSITE de anchors) o del propietario (re-auditoría, PEND-004/006). **Verificar primero** (la disciplina del hilo) antes de construir. Se le puede preguntar al propietario cuál priorizar, o parar aquí: el bloque de rotación + [9] es un punto de cierre limpio.

...
**Próximo incremento recomendado**: **ADR de secret-manager** (cierra el último residual de «Secretos/cadena» del §5; artefacto de diseño, sin superficie de runtime). _Alternativas_: security review formal (F6), o no-negatividad de motor a nivel BD (columna + trigger; OJO: las cuentas transitorias SÍ van negativas — diseñarlo sin romperlas). **Verificar primero** (la disciplina del hilo) antes de construir.

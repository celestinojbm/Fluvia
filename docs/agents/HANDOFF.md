...
**Próximo incremento recomendado**: **rotación EFECTIVA de `MFA_SECRET_KEY`** (secreto TOTP) — MISMO patrón AES-GCM que webhook-enc (keyring de descifrado por trial + re-cifrado de `users.totp_secret_enc`/`totp_pending_secret_enc`), reutilizando el enfoque probado. Luego el **pepper de API keys** (`API_KEY_HMAC_SECRET`, one-way — mecánica DISTINTA: multi-pepper, las keys viejas conservan el suyo hasta re-emitirse). _Alternativas mayores_: **security review formal** (F6), o **no-negatividad de motor a nivel BD** (columna + trigger; OJO: las cuentas transitorias SÍ van negativas). **Verificar primero** (la disciplina del hilo) antes de construir.

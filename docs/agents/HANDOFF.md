...
**El ADR de secret-manager quedó HECHO** (ADR-0012, **Propuesto**) — sobre la rama APILADA `claude/secret-manager-adr` (PR nuevo apilado sobre el del anclaje). Decide el enfoque agnóstico de proveedor: inyección de secretos como env al arranque (la app conserva su contrato de env validado + anti-mezcla; cero SDK en runtime, cero lock-in), imagen sin secretos, rotación por clave versionada (patrón v1→v2 de API keys). Artefacto de DISEÑO (Nivel A: sin runtime en sandbox; vendor + integración al desplegar, F5). Actualiza la regla 6 de `secrets-management.md` y el residual «Secretos/cadena» del §5.

**Próximo incremento recomendado**: **runbooks de rotación EFECTIVOS** para pepper/MFA-key/webhook-enc (clave versionada + re-hash/re-cifrado perezoso, como el v1→v2 de API keys) — el residual que ADR-0012 dejó explícito. _Alternativas_: **security review formal** (F6, pata mayor), o **no-negatividad de motor a nivel BD** (columna + trigger; OJO: las cuentas transitorias SÍ van negativas — diseñarlo sin romperlas). **Verificar primero** (la disciplina del hilo) antes de construir.

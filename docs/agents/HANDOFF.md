...
**Próximo incremento recomendado**: con las TRES patas de rotación cerradas, las opciones mayores de F6 son: **security review formal** (una pasada de auditoría de seguridad completa sobre el sistema endurecido), o **no-negatividad de motor a nivel BD** (columna + trigger sobre `ledger_accounts`/proyección; OJO: las cuentas transitorias SÍ van negativas — el diseño debe distinguirlas, no es trivial), o **load-soak-chaos** (pruebas de carga/resiliencia). El resto del residual §5 está acoplado a infra/despliegue (F5: cifrado field-level de credenciales de proveedor; `trustProxy` acotado y demás → PEND-006) o es del operador (copia OFFSITE de anchors). **Verificar primero** (la disciplina del hilo) antes de construir. Pregunta al propietario cuál priorizar.

...
| R-02 | Duplicación de efectos por retry/carrera | Financiero | Baja | Crítico | Ambas capas probadas: ledger (F2-08) y API (F2-09/10: claim en la misma tx que el efecto, crash pre/post-COMMIT, carrera N→1, property efectos==1) — Gate Idempotencia 🟢 técnico. Residual: cablear los endpoints de pagos F3-02 con esta capa (patrón ya probado en HTTP) | Mitigado |
